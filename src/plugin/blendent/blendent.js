    ...
    _blendent = {
      ...
      getColorNumber(color, flag = true) {
        return color
          .split(/[hsl\(,rgb\(,\,\s,,\)]/i)
          .reduce(
            (sum, item) => (item ? [...sum, flag ? (item.match("%") ? item.slice(0, -1) : item) : item] : sum),
            []
          );
      },
      ...
      similarity(color, number = 1, count = 12) {
        ...
        while (number) {
          ...
          arr.unshift(
            color - chunkAngle * number < 0 ? color - chunkAngle * number + 360 : color - chunkAngle * number
          );
          number--;
        }
        return arr;
      ...
      split(color, flag = true, count = 12) {
        ...
        return [
          color,
          flag
            ? color + chunkAngle * 2 > 360
              ? color + chunkAngle * 2 - 360
              : color + chunkAngle * 2
            : color - chunkAngle * 2 < 0
            ? 360 + color - chunkAngle * 2
            : color - chunkAngle * 2,
          color + chunkAngle + 180 > 360 ? color + chunkAngle - 180 : color + chunkAngle + 180
        ];
      },
      ...
      doubleSplit(color, flag = true, count = 12) {
        ...
        return [
          ...
          flag
            ...
            ? 360 + color + chunkAngle * 2
            : color - chunkAngle * 2,
          flag ? (color + 180 > 360 ? color - 180 : color + 180) : color - 180 < 0 ? color + 180 : color - 180,
          flag
            ? color + chunkAngle * 2 + 180 > 360
              ? color + chunkAngle * 2 - 180
              : color + chunkAngle * 2 + 180
            : color + chunkAngle * 2 - 180 < 0
            ? color + chunkAngle * 2 + 180
            : color + chunkAngle * 2 - 180
        ];
      },
      ...
      square(color) {
        ...
        return [
          color,
          color + chunkQuarter > 360 ? color + chunkQuarter - 360 : color + chunkQuarter,
          color + 180 > 360 ? color - 180 : color + 180,
          color + chunkQuarter + 180 > 360 ? color + chunkQuarter - 180 : color + chunkQuarter + 180
        ];
      }
    };
